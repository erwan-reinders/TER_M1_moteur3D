...
function buildPipelines() {
    ...
    createValueSlider_UI("ambiant", blinnPhongRenderer, "ambiant", 0.0, 1.0, 0.1);
    ...
    p.addShader(new TextureGBuffer(shaders.get("textureGBuffer"), canvas.width*0.5, canvas.height*0.5));
    p.addShader(blinnPhongRenderer);
    p.addShader(gammaCorrectionRenderer);
    ...
    p.addShader(new ApplyToScreen(shaders.get("applyToScreen"), "BlinnPhong"        , w * 3.0 , startH, w, h));
    p.addShader(new ApplyToScreen(shaders.get("applyToScreen"), "BlinnPhongCorrected"));
    pipelines.push(p);
